#!/bin/bash
# FaceUnlock 설치 도우미 — DMG 사용자용 원클릭 설치.
#
# 이 파일 하나가 설치의 전부를 처리한다:
#   1) 앱을 /Applications 로 복사 (이미 드래그했다면 건너뜀)
...
#   4) 앱 실행
#
# DMG 에서는 '설치 도우미.command' 라는 이름으로 들어간다.
set -euo pipefail
...
if [ -d "$APP_DST" ]; then
    ...
    step "앱을 Applications 로 복사합니다"
fi
if [ ! -d "$APP_SRC" ]; then
    echo "오류: 이 파일 옆에 FaceUnlock.app 이 없습니다. DMG 안에서 실행해 주세요." >&2
    exit 1
fi
...
if [ -d "$DEST/ArcFace.mlpackage" ]; then
    ...
    echo "    라이선스상 DMG 에 동봉할 수 없어 공식 배포처에서 직접 내려받습니다."
    ...
    if ! find_python; then
        ...
        if [ -n "$BREW" ]; then
            echo ""
            echo "    변환 도구가 Python 3.12 를 필요로 합니다 (설치되어 있지 않음)."
            read -r -p "    Homebrew 로 지금 설치할까요? [Y/n] " reply
            case "$reply" in
                [nN]*) echo "    설치를 중단합니다. python@3.12 설치 후 다시 실행해 주세요."; exit 1 ;;
                *)     "$BREW" install python@3.12 ;;
            esac
            find_python || { echo "오류: Python 설치 후에도 찾지 못했습니다." >&2; exit 1; }
        else
            echo "오류: Python 3.11/3.12 와 Homebrew 가 모두 없습니다." >&2
            echo "      https://brew.sh 에서 Homebrew 설치 후 이 파일을 다시 실행해 주세요." >&2
            exit 1
        fi
    fi
    echo "    Python: $PY"
...
# ── 4. 실행 ──────────────────────────────────────────────────
step "설치 완료 — FaceUnlock 을 실행합니다"
open "$APP_DST"

echo ""
echo "✅ 끝났습니다. 메뉴바의 얼굴 아이콘 → 설정에서 얼굴과 비밀번호를 등록하세요."
echo "   이 창은 닫아도 됩니다."
